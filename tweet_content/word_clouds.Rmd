...
twitter_merged_noRT_clean <- removeNumbers(twitter_merged_noRT$text) 
twitter_merged_noRT_clean <- gsub("@\\w+","",twitter_merged_noRT_clean)
twitter_merged_noRT_clean <- gsub("http:\\w+","",twitter_merged_noRT_clean)
twitter_merged_noRT$text <- twitter_merged_noRT_clean
...
## top 100 based on likes
```{r}
top_100_noRT_likes <- twitter_merged_noRT %>% 
...
# top 100 based on likes
```{r,echo=F,warning=F,message=F}
create_wordcloud(top_100_noRT_likes, "")
```
...
### top 100 based on likes filtered by "soil
```{r,echo=F,warning=F,message=F}
create_wordcloud(top_100_noRT_likes, "soil")
```
