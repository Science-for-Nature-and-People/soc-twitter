...
```{r}
RT_clean <- clean_data(RT, rm_pope = F, rm_india = F)
...
test <- routes_tidy %>% 
  ...
  mutate(node_label = if_else(centrality > 350, label, "")) %>% 
  mutate(node_size = if_else(centrality > 350, centrality, 0)) 
...
ggraph(test, layout = 'linear', circular = TRUE, sort.by = 'community') + 
  ...
  labs(title="R Twitter Activity", 
       subtitle="Retweets and mentions between key members of the community") +
  theme_graph() +
  guides(edge_width = FALSE, 
         edge_colour = guide_legend(title = NULL,
...
ggraph(test, layout = 'linear', circular = TRUE, sort.by = 'ranking') + 
  geom_edge_arc(alpha=0.05, aes(col = factor(weight), edge_width = weight)) +
  geom_node_label(aes(label=node_label, size=node_size, col = factor(community)),
                  label.size=0, fill="#ffffff66", segment.colour="slateblue",
                  color="black", repel=TRUE, fontface="bold", 
                  show.legend = FALSE) +
  coord_fixed() +
  scale_size_area(trans="sqrt") +
