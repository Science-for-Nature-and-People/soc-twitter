...
# select the noRT with retweet count > 0
...
find_rt <- function(rank, noRT_dataset, RT_dataset) {
  ...
  value <- result.df$Date - date(noRT_dataset$created_at[rank])
  if(value > 0){result.df$time_since =  value}
  else{result.df$time_since = result.df$Date - result.df$Date[1]}
...
test$Cat <- 1
g <- ggplot() + geom_line(data = test, aes(x = Time_since, y = Number)) + xlim(0,3) + 
...
while(i < nrow(SHS19_n)){
  ...
  tmp$Cat <- i
  g <- g + geom_line(data = tmp, aes(x = Time_since, y = Number)) + xlim(0,3)
  test <- rbind(test, tmp)
  i = i + 1
}
