...
ggraph(top100_labs, layout = 'linear', circular = TRUE) + 
...
### network for top 100 tweets, including india
```{r, fig.width=12, fig.height=12}
RT_clean <- clean_data(RT, rm_pope = T, rm_india = F)
...
india_graph <- tbl_graph(nodes = nodes, edges = edges, directed = TRUE)


top_india <- india_graph %>% 
  mutate(community = group_walktrap(weights = weight)) %>% 
  mutate(centrality = centrality_degree(weights = weight)) %>% 
  arrange(community, centrality) 

india_labs <- top_india %>% 
  mutate(ranking = row_number()) %>% 
  dplyr::top_n(500, centrality) %>% 
  mutate(node_label = if_else(centrality > 150, label, "")) %>% 
  mutate(node_size = if_else(centrality > 150, centrality, 0)) %>% 
...
ggraph(india_labs, layout = 'linear', circular = TRUE) + 
  geom_edge_arc(alpha=0.05, 
                aes(col = ifelse(weight > 1, ">1", "1"), 
                    edge_width = weight)) +
  geom_node_label(aes(label=node_label, 
                      size=node_size, 
                      col = factor(community)),
...
### network for specific user groups
```{r, fig.width=12, fig.height=12}
RT_clean <- clean_data(RT, rm_pope = T, rm_india = F)
noRT_clean <- clean_data(noRT, rm_pope = T, rm_india = F)
...
farmer_tweets <- noRT_clean %>% 
  filter(screen_name %in% farmers$screen_name) %>% 
  select(text, screen_name) %>% 
  dplyr::rename("author" = "screen_name")
...
joined <- dplyr::left_join(retweets, farmer_tweets)
...
india_labs <- top_india %>% 
  mutate(ranking = row_number()) %>% 
  dplyr::top_n(500, centrality) %>% 
  mutate(node_label = if_else(centrality > 50, label, "")) %>% 
  mutate(node_size = if_else(centrality > 50, 10, 0)) 
...
ggraph(india_labs, layout = 'linear', circular = TRUE) +
  ...
  coord_fixed() +
  scale_size_area(trans="sqrt") +
  labs(title="Retweet Network for farmers among top users",
       subtitle="most central users associated with the top farmers") +
  theme_graph() +
  guides(edge_width = FALSE,
         edge_colour = guide_legend(title = "Edge weights",
                                    override.aes = list(edge_alpha = 1))) +
...
ggraph(india_labs, layout = "graphopt") + 
  geom_node_point() +
...
ggraph(test, layout = "graphopt") + 
  ...
  geom_node_label(aes(label= node_label, 
                  ...
                  fontface="bold", 
                  show.legend = FALSE) +
  labs(title = "filtered for centrality > 1") +
  theme_graph()
```
