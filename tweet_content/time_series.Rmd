...
soil_health_tweets <- input %>% 
  filter(
    str_detect(tolower(text), paste(c("soil health","#soilhealth","healthy soil"), collapse = '|')))
...
p <- ggplot(hit_split, aes(month(created_at), y = hits)) +
  ...
  scale_fill_gradient(low = "white", high = "steelblue", name = "retweet count (log scale)", trans = 'log') +
  labs(title = "Heat Map", x = "month", y = "query term") 
  
p + theme_grey(base_size = base_size)
```
```{r}
# separate by categories
# soil health, soil quality, soil fertility, rangeland health

# mutate the 4 categories in one data frame
all.df <- soil_health_tweets %>% bind_rows(soil_quality_tweets) %>% bind_rows(soil_fertility_tweets) %>% bind_rows(rangeland_health_tweets)

#split the hit terms
hit_split.df <- separate_rows(all.df, hits, sep = ";") %>% filter(!is.na(hits))

#day
#all.df$created_at <- date(all.df$created_at)
#month
all.df$created_at <- month(all.df$created_at)

ggplot(hit_split.df, aes(y = retweet_count, x = date(created_at), color = hits)) + geom_line() + ggtitle('time series')
