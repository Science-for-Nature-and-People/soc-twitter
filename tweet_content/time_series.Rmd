...
p_17 <- ggplot(hit_split%>% filter(created_year == 2017), aes(week(created_at), y = hits)) +
  geom_tile(aes(fill = log(retweet_count)), color = "white") +
  scale_fill_gradient(low = "white", high = "steelblue", name = "retweet count (log scale)", 
                      limit = c(log(min_rt),log(max_rt))) + 
...
ggplot(hit_split, aes(y = log(retweet_count), x = date(created_at), color = hits)) + 
  geom_line() + ggtitle('time series')

ggplot(hit_split, aes(y = log(retweet_count), x = week(created_at), fill = hits)) +
...
b_17 <- ggplot(hit_split %>% filter(created_year == 2017), 
               aes(y = log(retweet_count), x = week(created_at), fill = hits)) + 
        geom_bar(stat = "identity") + ggtitle('time series 2017')
...
count_top_100 <- rbind(c17, c18, c19, size = "last")
grid.newpage()
grid.draw(count_top_100)
```
