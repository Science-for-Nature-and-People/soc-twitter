...
ggplot(hit_sum, aes(hit, count)) +
  ...
  coord_flip() +
  labs(title = "number of hits on each query term") +
  theme_bw() +
  theme(legend.position="none") 
...
selected_pct_full <- selected_words %>% 
  mutate(quality = round((quality/sum_qual)*100,2),
         range = round((range/sum_range)*100,2),
         fertility = round((fertility/sum_fertil)*100,2),
         health = round((health/sum_health)*100,2)) %>% 
  gather("query", "percent", -word)
