...
#   arrange(-retweet_count) %>%
#   filter(screen_name != "Pontifex") #remove the pope..
>>>>>>> 399dfbf26cafb036aef21d25500561e8e82b3ea1
...
#refactoring to order in visualization
...
#refactoring to order in visualization
count_noSoil$query <- factor(count_noSoil$query, levels = count_noSoil$query[order(count_noSoil$counts)])

ggplot(count_noSoil, aes(x = query, y = counts)) +
  geom_col(aes(fill = query)) + 
  geom_text(aes(label = counts), nudge_y = 100) +
    labs(
    title = "number of hits on each search term (excluding 'soil' terms)",
    y = 'search term'
  ) +
  theme_bw() +
  theme(legend.position="none") +
...
ggplot(hit_sum, aes(hit, count)) +
  ...
  geom_text(aes(label = count), nudge_y = 1750) +
...
#filter terms that are all related to 'soil healt'
...
#join word lists to get relative counts of each all the terms found within each df
...
## create word list of top words from each - this will then be used as a filter from the larger total list above
...
# join all top words from each category into a word list
...
#### same comparison, but visually comparing based on word counts of 'range'
```{r, echo=F, fig.height=10, fig.width=12}
